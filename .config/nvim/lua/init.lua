...
local on_attach = function(client, bufnr)
    ...
    -- vim.api.nvim_buf_set_keymap(bufnr, 'n', '<leader>f', [[<cmd>lua require'telescope.builtin'.lsp_document_symbols{symbol_width=50}<CR>]], opts)
...
-- vim.api.nvim_set_keymap('n', '<f7>', ":Copilot disable<CR>", { noremap = true, silent = false })
...
-- enter vim substitute for current word
vim.keymap.set("n", "<leader>S", ":%s/\\<<C-r><C-w>\\>//g<left><left>", { silent = false })
vim.keymap.set("n", "<leader>s", ":s/\\<<C-r><C-w>\\>//g<left><left>", { silent = false })

-- mapping to call lsp rename
vim.keymap.set("n", "<leader>r", "<cmd>lua vim.lsp.buf.rename()<CR>", { silent = false })
